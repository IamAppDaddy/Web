...
function initCoinSoundPool() {
  for (var i = 0; i < maxCoinSounds; i++) {
    ...
    sound.volume = 0.5;
    sound.preload = "auto";
    coinSoundPool.push(sound);
  }
}

function playCoinSound() {
  var sound = coinSoundPool[coinSoundIndex];
  if (sound.paused || sound.currentTime === 0 || sound.ended) {
    sound.currentTime = 0;
    sound.play().catch(function (error) {
      console.error("Coin audio play failed:", error);
    });
  }
  coinSoundIndex = (coinSoundIndex + 1) % maxCoinSounds;
}

function initEnemySoundPool() {
  for (var i = 0; i < maxEnemySounds; i++) {
    var soundFile = i % 2 === 0 ? "sounds/dash1.wav" : "sounds/dash2.wav";
    var sound = new Audio(soundFile);
    sound.volume = 0.5;
    sound.preload = "auto";
    enemySoundPool.push(sound);
  }
}

function playEnemySound() {
  var randomIndex = Math.floor(Math.random() * maxEnemySounds);
  var sound = enemySoundPool[randomIndex];
  if (sound.paused || sound.currentTime === 0 || sound.ended) {
    sound.currentTime = 0;
    sound.play().catch(function (error) {
      console.error("Enemy audio play failed:", error);
    });
  }
}

function initPlaneSounds() {
  planeStartSound = new Audio("sounds/plane_start.mp3");
  planeStartSound.volume = 0.5;
  planeStartSound.preload = "auto";

  planeMiddleSound = new Audio("sounds/plane_middle.mp3");
  planeMiddleSound.loop = true;
  planeMiddleSound.volume = 0.5;
  planeMiddleSound.preload = "auto";

  planeEndSound = new Audio("sounds/plane_end.mp3");
  ...
  document.addEventListener("click", function startAudio() {
    ...
    planeStartSound.play().catch(function (error) {
      console.error("Plane start audio play failed:", error);
    });
...
function stopAllPlaneSoundsExceptMiddle() {
  ...
  planeEndSound.currentTime = 0;
}

function updatePlaneSound() {
  if (game.status === "playing" && planeMiddleSound) {
...
var Pilot = function () {
  ...
  var faceMat = new THREE.MeshLambertMaterial({ color: Colors.darkGreen });
  var face = new THREE.Mesh(faceGeom, faceMat);
  this.mesh.add(face);

  var hairGeom = new THREE.BoxGeometry(4, 4, 4);
  var hairMat = new THREE.MeshLambertMaterial({ color: Colors.camoBrown });
  ...
  var glassMat = new THREE.MeshLambertMaterial({ color: Colors.darkGray });
...
var AirPlane = function () {
  ...
  this.mesh.add(tailFin1);

  var tailFin2 = tailFin1.clone();
  tailFin2.position.z = -10;
  ...
  wings.scale.set(0.2, 1, 1);
  wings.rotation.z = Math.PI / 6;
  wings.castShadow = true;
  wings.receiveShadow = true;
  this.mesh.add(wings);

  var geomCockpit = new THREE.SphereGeometry(10, 32, 32);
  ...
  wheelL.position.z = -20;
  this.mesh.add(wheelL);

  var wheelB = wheelR.clone();
  wheelB.scale.set(0.6, 0.6, 0.6);
  wheelB.position.set(-40, -10, 0);
  this.mesh.add(wheelB);

  this.pilot = new Pilot();
  this.pilot.mesh.position.set(0, 20, 0);
...
EnnemiesHolder.prototype.rotateEnnemies = function () {
  for (var i = 0; i < this.ennemiesInUse.length; i++) {
    ...
    if (d < game.ennemyDistanceTolerance) {
      ...
      planeStartSound.play().catch(function (error) {
        console.error("Plane start audio on collision failed:", error);
      });
      planeStartSound.onended = function () {
        planeMiddleSound.play().catch(function (error) {
          console.error("Plane middle audio after collision failed:", error);
        });
      };
...
function loop() {
  ...
  if (game.status == "playing") {
    ...
    updateEnergy();
    updatePlaneSound();
    game.baseSpeed += (game.targetBaseSpeed - game.baseSpeed) * deltaTime * 0.1;
    game.speed = game.baseSpeed * game.planeSpeed;
  } else if (game.status == "gameover") {
    if (!planeEndSound.playedOnce) {
      planeMiddleSound.pause();
      stopAllPlaneSoundsExceptMiddle();
    ...
    planeEndSound.playedOnce = false;
  }

  airplane.fan.rotation.x += 0.3 + game.planeSpeed * deltaTime * 0.005;
